	...
	if(file_exists($tempFile)) {
		if(isImage($tempFile) && filesize($tempFile) > 5000) {
			...
			if($fileExt != 'jpg' && $fileExt != 'png') {
				shell_exec("mogrify -strip -format png \"{$tempFile}\"");
				$fileExt = 'png';
			}
...
function matchImage($folderPath, $gameName) {
	...
	if(isset($localImageArray[$folderPath]['simple'][safeName($gameName)])) {
		return "{$folderPath}/{$localImageArray[$folderPath]['files'][$localImageArray[$folderPath]['simple'][safeName($gameName)]]}";
	}
}
...
function getGameInfo($thisRom, $setParts, $thisSet) {
	...
	if($setParts['assets'] == 'MAME') {
		// Box Art
		if(!$romImage) $romImage = matchImage(ASSET_ROOT."/{$setParts['assets']}/Advert", $thisRom['name']);
		if(!$romImage) $romImage = matchImage(ASSET_ROOT."/{$setParts['assets']}-Extra/flyer", $thisRom['name']);
		if(!$romImage) $romImage = matchImage(ASSET_ROOT."/{$setParts['assets']}/Title", $thisRom['name']);
		if(!$romImage) $romImage = matchImage(ASSET_ROOT."/{$setParts['assets']}-Extra/titles", $thisRom['name']);
		// Wheel Art
		if(!$romWheelImage) $romWheelImage = matchImage(ASSET_ROOT."/{$setParts['assets']}/Logos", $thisRom['name']);
		if(!$romWheelImage) $romWheelImage = matchImage(ASSET_ROOT."/{$setParts['assets']}-Extra/logo", $thisRom['name']);
		if(!$romWheelImage) $romWheelImage = matchImage(ASSET_ROOT."/{$setParts['assets']}/Marquee", $thisRom['name']);
		if(!$romWheelImage) $romWheelImage = matchImage(ASSET_ROOT."/{$setParts['assets']}-Extra/marquees", $thisRom['name']);
	}

	// Find a box image
	if(!$romImage) $romImage = matchImage(ASSET_ROOT."/{$setParts['assets']}/Box", $thisRom['name']);
	if(!$romImage) $romImage = matchImage(ASSET_ROOT."/{$setParts['assets']}/Box", $thisRom['niceName']);
	if(!$romImage) $romImage = matchImage(ASSET_ROOT."/{$setParts['assets']}/Box", $thisRom['safeName']);

	// Fall back to 3D box images (they don't look as good)
	#if(!$romImage) $romImage = matchImage(ASSET_ROOT."/{$setParts['assets']}/Box_3D", $thisRom['name']);
	#if(!$romImage) $romImage = matchImage(ASSET_ROOT."/{$setParts['assets']}/Box_3D", $thisRom['niceName']);
	#if(!$romImage) $romImage = matchImage(ASSET_ROOT."/{$setParts['assets']}/Box_3D", $thisRom['safeName']);

	// Find a snap video or image using MAME's own Extras
        if($setParts['assets'] == 'MAME') {
		if(!$romSnapImage) $romSnapImage = matchImage(ASSET_ROOT."/{$setParts['assets']}-Extra/videosnaps", $thisRom['name']);
		if(!$romSnapImage) $romSnapImage = matchImage(ASSET_ROOT."/{$setParts['assets']}-Extra/snap", $thisRom['name']);
	}

	// Find a snap / screenshot image
	if(!$romSnapImage) $romSnapImage = matchImage(ASSET_ROOT."/{$setParts['assets']}/Snap", $thisRom['name']);
	if(!$romSnapImage) $romSnapImage = matchImage(ASSET_ROOT."/{$setParts['assets']}/Snap", $thisRom['niceName']);
	if(!$romSnapImage) $romSnapImage = matchImage(ASSET_ROOT."/{$setParts['assets']}/Snap", $thisRom['safeName']);

	// Find a wheel image
	if(!$romWheelImage) $romWheelImage = matchImage(ASSET_ROOT."/{$setParts['assets']}/Logos", $thisRom['name']);
	if(!$romWheelImage) $romWheelImage = matchImage(ASSET_ROOT."/{$setParts['assets']}/Logos", $thisRom['niceName']);
	if(!$romWheelImage) $romWheelImage = matchImage(ASSET_ROOT."/{$setParts['assets']}/Logos", $thisRom['safeName']);

	// If extra name / location is provided, check that too
	if($setParts['assets2']) {
		if(!$romImage) $romImage = matchImage(ASSET_ROOT."/{$setParts['assets2']}/Box", $thisRom['name']);
		if(!$romSnapImage) $romSnapImage = matchImage(ASSET_ROOT."/{$setParts['assets2']}/Snap", $thisRom['name']);
		if(!$romWheelImage) $romWheelImage = matchImage(ASSET_ROOT."/{$setParts['assets2']}/Logos", $thisRom['name']);
	}

	// Check for versions we've already downloaded
	if(!$romImage) $romImage = matchImage(EXTRAASSET_ROOT."/{$setParts['assets']}/Box", $thisRom['name']);
	if(!$romImage) $romImage = matchImage(EXTRAASSET_ROOT."/{$setParts['assets']}/Box", $thisRom['niceName']);
	if(!$romSnapImage) $romSnapImage = matchImage(EXTRAASSET_ROOT."/{$setParts['assets']}/Snap", $thisRom['name']);
	if(!$romSnapImage) $romSnapImage = matchImage(EXTRAASSET_ROOT."/{$setParts['assets']}/Snap", $thisRom['niceName']);
	if(!$romWheelImage) $romWheelImage = matchImage(EXTRAASSET_ROOT."/{$setParts['assets']}/Wheel", $thisRom['name']);
	if(!$romWheelImage) $romWheelImage = matchImage(EXTRAASSET_ROOT."/{$setParts['assets']}/Wheel", $thisRom['niceName']);

	// Make the box folder in case it doesn't already exist before we try to work on it
	if(!$romImage) {
		$thisRom['missingImage'] = 1;
	}else{
		$thisRom['missingImage'] = false;
	}

	// Make the snap folder in case it doesn't already exist before we try to work on it
	if(!$romSnapImage) {
		$thisRom['missingSnapImage'] = 1;
	}else{
		$thisRom['missingSnapImage'] = false;
	}

	// Make the wheel folder in case it doesn't already exist before we try to work on it
	if(!$romWheelImage) {
		$thisRom['missingWheelImage'] = 1;
	}else{
		$thisRom['missingWheelImage'] = false;
	}

	// Extra checks requested
	if(FORCE_IMAGESCAN || isset($newGame)) {
		$thisRom['imageChecks'] = 1;
	}else{
		$thisRom['imageChecks'] = false;
	}

	if(!detailsComplete($thisRom['safeName'], $setParts['dbname'])) { $thisRom['missingDetails'] = true; }else{ $thisRom['missingDetails'] = false; }
	...
	if(!$gameDBOArr || $thisRom['missingDetails']) {
		...
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname']) && $setParts['assets'] == 'MAME') mameBinaryCheck($thisRom['niceName'], $thisRom['safeName'], $thisRom['name']);
		#if(!detailsComplete($thisRom['safeName'], $setParts['dbname']) && $setParts['assets'] == 'MAME') mamedbScrape($thisRom['niceName'], $thisRom['safeName'], $thisRom['name']);
		#if(!detailsComplete($thisRom['safeName'], $setParts['dbname']) && $setParts['assets'] == 'MAME') mameFileScrape($thisRom['niceName'], $thisRom['safeName'], $thisRom['name']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname']) && $setParts['assets'] == 'Commodore_Amiga') holScrape($thisRom['niceName'], $thisRom['safeName'], $setParts['dbname']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname']) && $setParts['assets'] == 'Commodore_Amiga_CD32') holScrape($thisRom['niceName'], $thisRom['safeName'], $setParts['dbname']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname']) && $setParts['assets'] == 'Doom') doomworldScraper($thisRom['safeName'], $thisRom['niceName']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname']) && $setParts['assets'] == 'PICO-8') lexaloffleScraper($thisRom['niceName'], $thisRom['safeName']);

		// General scrapers
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname'])) launchboxDbScraper($thisRom['safeName'], $setParts['dbname'], $thisRom['niceName'], $setParts['lbid'], $setParts['assets']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname'])) openvgdbScrape($thisRom['safeName'], $setParts['dbname'], $thisRom['niceName'], $setParts['ovgdb'], $setParts['assets']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname'])) thegamesdbScrape($thisRom['safeName'], $setParts['dbname'], $thisRom['niceName'], $setParts['tgdbid'], $setParts['assets']);
		#if(!detailsComplete($thisRom['safeName'], $setParts['dbname'])) archivevgScraper($thisRom['safeName'], $setParts['dbname'], $thisRom['niceName'], $setParts['archivevgname'], $setParts['assets']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname'])) mobyScrape($thisRom['safeName'], $setParts['dbname'], $thisRom['niceName'], $setParts['mobyid'], $setParts['assets']);
		#if(!detailsComplete($thisRom['safeName'], $setParts['dbname'])) allgamecomScraper($thisRom['safeName'], $setParts['dbname'], $thisRom['niceName'], $setParts['allgamename'], $setParts['assets']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname'])) ignScraper($thisRom['safeName'], $setParts['dbname'], $thisRom['niceName'], $setParts['ignname'], $setParts['assets']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname'])) rfgenScraper($thisRom['safeName'], $setParts['dbname'], $thisRom['niceName'], $setParts['rfgenid'], $setParts['assets']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname'])) giantbombScrape($thisRom['safeName'], $setParts['dbname'], $thisRom['niceName'], $setParts['gbid'], $setParts['assets']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname'])) gamefaqsScraper($thisRom['safeName'], $setParts['dbname'], $thisRom['niceName'], $setParts['gamefaqsid'], $setParts['assets']);

		// Normal platform specific scrapers
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname']) && $setParts['assets'] == 'Amstrad_CPC') amstradcpcwikiScraper($thisRom['niceName'], $thisRom['safeName']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname']) && $setParts['assets'] == 'Commodore_Amiga') amigaWhdloadXmlScraper($thisRom['safeName'], $thisRom['niceName']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname']) && $setParts['assets'] == 'Amstrad_CPC') cpcpowerFileScraper($thisRom['safeName'], $thisRom['niceName']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname']) && $setParts['assets'] == 'GCE_Vectrex') vectrexgdbScraper($thisRom['safeName'], $thisRom['niceName']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname']) && $setParts['assets'] == 'Philips_CD_i') pcdicomScraper($thisRom['safeName'], $thisRom['niceName'], $setParts['assets']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname']) && $setParts['assets'] == 'NEC_PC_FX') pcfxwikiScraper($thisRom['niceName'], $thisRom['safeName']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname']) && $setParts['assets'] == 'Sinclair_ZX_Spectrum') wosScraper($thisRom['safeName'], $thisRom['niceName'], $setParts['assets']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname']) && $setParts['assets'] == 'Sinclair_ZX_Spectrum') zxspectrumwikiScraper($thisRom['safeName'], $thisRom['niceName']);
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname']) && $setParts['assets'] == 'Touhou') touhouwikiaScraper($thisRom['safeName'], $thisRom['niceName'], $setParts['assets']);

		// Give up and store empty game in the DB
		if(!detailsComplete($thisRom['safeName'], $setParts['dbname'])) updateGameDetails('Dummy', $thisRom['niceName'], $thisRom['safeName'], $setParts['dbname']);
	}
	...
	if($thisRom['imageChecks'] && !$romImage) {
		if(!$romImage && isset($setParts['tgdbid'])) thegamesdbScrape($thisRom['safeName'], $setParts['dbname'], $thisRom['niceName'], $setParts['tgdbid'], $setParts['assets'], true);
		...
		if(!$romImage && isset($setParts['mobyid'])) mobyScrape($thisRom['safeName'], $setParts['dbname'], $thisRom['niceName'], $setParts['mobyid'], $setParts['assets'], true);
		if(!$romImage && isset($setParts['ignname'])) ignScraper($thisRom['safeName'], $setParts['dbname'], $thisRom['niceName'], $setParts['ignname'], $setParts['assets'], true);
		if(!$romImage && isset($setParts['allgamename'])) allgamecomScraper($thisRom['safeName'], $setParts['dbname'], $thisRom['niceName'], $setParts['allgamename'], $setParts['assets'], true);
		if(!$romImage && isset($setParts['gbid'])) giantbombScrape($thisRom['safeName'], $setParts['dbname'], $thisRom['niceName'], $setParts['gbid'], $setParts['assets'], true);

		// Get the wheel artwork (handled by the TGDB scraper)
		if(!$romWheelImage && isset($setParts['tgdbid'])) thegamesdbScrape($thisRom['safeName'], $setParts['dbname'], $thisRom['niceName'], $setParts['tgdbid'], $setParts['assets'], false, true);
		// Get the snap artwork (handled by the TGDB scraper)
		if(!$romSnapImage && isset($setParts['tgdbid'])) thegamesdbScrape($thisRom['safeName'], $setParts['dbname'], $thisRom['niceName'], $setParts['tgdbid'], $setParts['assets'], false, false, true);
	}
...
function getMameGameXml($romName) {
	...
	if(!$gameXml) {
		$gameXml = json_encode(simplexml_load_string(cleanShell("sdlmame -listxml '{$romName}' 2>/dev/null")));
		DBSimple("INSERT INTO mameGameXml SET gameMatchName = ?, mameXml = ?", array($romName, $gameXml));
	}
...
function populateLaunchboxDb() {
	...
	if(time() - @filemtime('resources/Metadata.xml') > 172800) {
			...
			foreach($launchboxArray as $thisLaunchboxGame) {
				if($thisLaunchboxGame['Platform']) {
					...
					DBSimple(
						...
						array($thisLaunchboxGame['DatabaseID'], $thisLaunchboxGame['Name'], safeName($thisLaunchboxGame['Name']), $thisLaunchboxGame['ReleaseYear'], $thisLaunchboxGame['MaxPlayers'], round($thisLaunchboxGame['CommunityRating'], 1), $thisLaunchboxGame['Overview'], $thisLaunchboxGame['Platform'], $thisLaunchboxGame['Developer'], $thisLaunchboxGame['Publisher'], $thisLaunchboxGame['Genres'], $lbImageArray[$thisLaunchboxGame['DatabaseID']]['Box - Front'], $lbImageArray[$thisLaunchboxGame['DatabaseID']]['Clear Logo'], $lbImageArray[$thisLaunchboxGame['DatabaseID']]['Screenshot - Gameplay'])
					);
				}
			}
		}else{
