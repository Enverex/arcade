...
while($pageNumber <= 10) {
	...
	$thisPageContent = getPage("https://www.lexaloffle.com/bbs/?page={$pageNumber}&mode=carts&cat=7&sub=2&orderby=rating");
	$rawLines = preg_match('%[\s]pdat=\[([\s\S]+?)\];%', $thisPageContent, $rawArrayLines);
	$rawArray = explode("\n", $rawArrayLines[1]);
	...
	foreach($rawArray as $thisRow) {
		...
		// Get some more info
		$gameActualPage = getPage("https://www.lexaloffle.com/bbs/?pid={$gameId}");
		...
		// Slashes are bad, colons and exclamations are bad
		...
		// Create a basic version of the authors name to use in the filename.
		// The GID seems to change occasionally so it's not much use here.
		$cleanAuthorName = safeName($authorName);

		// Games may contain special characters so escape them
		$gamePath = escapeshellarg("{$gameDir}/{$gameName} ({$cleanAuthorName}).png");
		$imagePath = escapeshellarg("{$assetDir}/{$gameName} ({$cleanAuthorName}).png");

		$newGameInsert = DBSimple("INSERT IGNORE INTO picoGames SET pico_gid = ?, pico_author = ?, pico_name = ?, pico_date = ?", array($gameId, $authorName, $gameName, $gameDate));

		if($newGameInsert) {
			// Only download them if they don't already exist
			if(!file_exists($gamePath)) {
				echo "Downloading {$gameName}\n";
				shell_exec("{$wgetLocal} 'https://www.lexaloffle.com/bbs/cposts/{$gameIdPrefix}/{$gameId}.p8.png' -O {$gamePath}");
			}else{
				echo "We've already got {$gameName}. Skipping.\n";
			}

			if(!file_exists($imagePath)) {
				shell_exec("{$wgetLocal} 'https://www.lexaloffle.com/bbs/thumbs/pico{$gameId}.png' -O {$imagePath}");
			}
		}
	}
	$pageNumber++;
